...
pub async fn oidc_login(State(state): State<AppState>, session: Session) -> impl IntoResponse {
    ...
    let client = CoreClient::from_provider_metadata(metadata.clone(), client_id, client_secret);
    ...
    let _: () = session
        .insert("oidc_pkce_verifier", pkce_verifier)
        .await
        .unwrap_or_else(|e| error!("Session error: {}", e));
    let _: () = session
        .insert("oidc_state", csrf_token.secret().clone())
        .await
        .unwrap_or_else(|e| error!("Session error: {}", e));
    let _: () = session
...
async fn oidc_http_client(
    ...
    let mut http_response = openidconnect::http::Response::builder().status(status_code);
...
pub async fn oidc_callback(
    ...
    let token_request = match client.exchange_code(AuthorizationCode::new(params.code.clone())) {
    ...
    let token_response: openidconnect::core::CoreTokenResponse =
        match token_request.request_async(&oidc_http_client).await {
            Ok(res) => res,
            Err(e) => {
    ...
    let tenant_id = claims_json["tenant_id"]
        .as_str()
        .map(|s| s.to_string())
        .unwrap_or_else(|| "default".to_string());
    let user_roles = claims_json["roles"]
        .as_array()
        .map(|arr| {
            arr.iter()
    ...
    let _ = session
        .remove::<PkceCodeVerifier>("oidc_pkce_verifier")
        .await;
