use axum::{
    ...
    http::header::{CACHE_CONTROL, CONTENT_SECURITY_POLICY, X_FRAME_OPTIONS},
};
...
pub fn create_internal_router(state: AppState) -> Router {
    let security_layer = ServiceBuilder::new()
        .layer(SetResponseHeaderLayer::overriding(CACHE_CONTROL, "no-store, max-age=0".parse().unwrap()))
        .layer(SetResponseHeaderLayer::overriding(CONTENT_SECURITY_POLICY, "default-src 'self'; script-src 'self' https://unpkg.com; style-src 'self' 'unsafe-inline';".parse().unwrap()))
