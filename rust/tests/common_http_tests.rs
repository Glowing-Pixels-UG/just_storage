...
fn test_http_helpers_request_builders() {
    ...
    assert_eq!(post.headers().get("content-type").unwrap(), "application/json");
    ...
    assert_eq!(auth_json.headers().get("authorization").unwrap(), "Bearer key");
    assert_eq!(auth_json.headers().get("content-type").unwrap(), "application/json");
}
