...
async fn setup_test_environment(
    ...
    Arc<UploadObjectUseCase>,
    Arc<DownloadObjectUseCase>,
    Arc<DeleteObjectUseCase>,
    TempDir,
) {
    // Setup storage
...
async fn sqlx_test_error_cases(pool: PgPool) {
    let (upload_use_case, _download_use_case, _delete_use_case, _temp_dir) =
        setup_test_environment(&pool).await;
...
async fn sqlx_test_concurrent_operations(pool: PgPool) {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let object = upload_uc
                .execute(request, reader)
                .await
                .expect("Concurrent upload failed");
