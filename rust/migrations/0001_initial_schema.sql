...
-- Partial unique index for key per tenant/namespace (only when key is not null and not deleted)
CREATE UNIQUE INDEX unique_key_per_tenant_ns
...
-- Note: Full text search on JSONB metadata would require a different approach (e.g., generated columns or triggers)
...
-- API KEYS TABLE
-- ============================================================================
-- Stores API keys for programmatic access, primarily for SaaS/multi-tenant scenarios.
...
CREATE TABLE api_keys (
    ...
    -- Key details
    api_key         TEXT NOT NULL UNIQUE,       -- The actual API key string
    tenant_id       TEXT NOT NULL,              -- Associated tenant/org
    name            TEXT NOT NULL,              -- Human-readable name (e.g., "Production API Key")
    description     TEXT,                       -- Optional description

    -- Permissions (for future use)
    permissions     JSONB DEFAULT '{}',         -- e.g., {"read": true, "write": true, "delete": true}
    ...
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    last_used_at    TIMESTAMPTZ,                -- Updated on successful authentication

    -- Constraints
    CHECK (length(api_key) >= 32),              -- Minimum key length
